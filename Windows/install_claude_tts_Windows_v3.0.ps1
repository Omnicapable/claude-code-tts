# =============================================================================
# install_claude_tts_Windows_v3.0.ps1  v3.0
# One-shot installer for Claude Code local TTS using Kokoro ONNX.
# Fully offline after install. No API keys. No data sent to third parties.
#
# Requirements: Windows 10/11, Python 3.9+, Claude Code installed
...
Write-Host ""
Write-Host "============================================"
Write-Host " Claude Code TTS Installer (Kokoro) v$version"
Write-Host "============================================"
Write-Host ""
...
Write-Host "[5/8] Writing Python scripts..."
...
# preview_voices.py
Set-Content -Path "$kokoro\preview_voices.py" -Encoding UTF8 -Value @'
# -*- coding: utf-8 -*-
# preview_voices.py - Cycle through Kokoro voices so you can hear them before choosing.
# Usage: py -3 preview_voices.py              (all voices, ~3 min)
#        py -3 preview_voices.py --category   (one per group, ~30 sec)
#        py -3 preview_voices.py am_onyx      (single voice test)
...
'@
Write-Host "[6/8] Writing PowerShell scripts..."
...
Write-Host "[1/4] Stopping TTS server and hotkey daemon..."
...
Write-Host "[2/4] Removing auto-start entries..."
...
Write-Host "[3/4] Removing hook from settings.json..."
...
Write-Host "[4/4] Removing TTS files..."
`$filesToRemove = @(
...
When the user asks "quick preview voices", "preview all voices", "preview voice <name>", or similar direct preview commands, run:
    py -3 $kokoro\tts_preview.py "<user request>"
Examples:
    py -3 $kokoro\tts_preview.py "quick preview voices"
    py -3 $kokoro\tts_preview.py "preview all voices"
    py -3 $kokoro\tts_preview.py "preview voice onyx"
...
Write-Host "[7/8] Updating Claude Code settings.json..."
$settingsPath = "$claude\settings.json"
if (Test-Path $settingsPath) {
    $settings = Get-Content $settingsPath -Raw | ConvertFrom-Json
} else {
...
Write-Host "[8/8] Setting up auto-start and launching server..."
...
Write-Host "============================================"
Write-Host ""
Write-Host " Version: v$version  |  Voice: am_onyx  |  Speed: 1.1x"
Write-Host ""
